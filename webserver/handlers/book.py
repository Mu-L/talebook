...
class BookUploadComplete(BookUploadBase):
    ...
    def post(self):
        ...
        try:
            ...
            # 用 basename(name) 重新拼接净化路径删除（与普通上传 os.remove(upload_path)
            # 的构造方式一致，均经 os.path.basename 净化，CodeQL 认可安全）
            cleanup_path = os.path.join(CONF["upload_path"], os.path.basename(name))
